...
const SearchCountry = (props) => {
  ...
  const originClickHandler = async () => {
    const res = await fetch(
      `https://cms.fasttrackvisa.com/api/${props.ce_name}/origin-country`
    );
    if (!res.ok) {
      setError("No country");
    }
    ...
    if (val !== "") {
      const filterData = originCountry.filter((el, index) => {
        if (el.name.toLowerCase().startsWith(val)) {
          ...
          return el.name
            .toLowerCase()
            .startsWith(val.substring(0, currentIndex));
        }
      });
  ...
  const selectedOriginCountry = async (e) => {
    ...
    try {
      const res = await fetch(
        `https://cms.fasttrackvisa.com/api/${props.ce_name}/origin-country/${country}`
      );
      if (!res.ok) {
        throw new Error();
      }
    ...
    if (inputVal !== "") {
      const filterData = destination.filter((el, index) => {
        if (el.name.toLowerCase().startsWith(inputVal)) {
          ...
          return el.name
            .toLowerCase()
            .startsWith(inputVal.substring(0, desCurrentIndex));
        }
      });
  ...
  const getErrorStatus = async (ext, conunty) => {
    axios
      .get(
        `${process.env.NEXT_PUBLIC_BASE_URL}/api/${ext}/international-visa/${conunty}`
      )
      .then(function () {
        ...
        <div className="origin_country">
          <div className="ftv-field">
            ...
            <input
              ...
              placeholder="Select..."
              onClick={originClickHandler}
              onChange={originChangekHandler}
              onKeyDown={handleKeyDown}
            />
          </div>
          <ul
            className={`country_list  ${isOpen === true ? "show" : "hidden"}`}
            ref={originRef}
          >
            {filterOrigin.map((country) => {
              return (
                <li
                  label={country.extention}
                  key={country.extention}
                  onClick={selectedOriginCountry}
                >
                  {country.name}
                </li>
              );
        ...
        <div className="destination_country" ref={destinationRef}>
          <div className="ftv-field">
            ...
            <input
              ...
              value={desInputValue || ""}
              placeholder="Select..."
              onClick={desClickHandler}
              onChange={desChangeHandler}
            />
          </div>
          <ul
            className={`country_list ${isActive === true ? "show" : "hidden"}`}
          >
            {filterDestination.map((des, i) => {
              return (
                <li key={i} onClick={desCountryClick}>
                  {des.name}
                </li>
              );
            })}
